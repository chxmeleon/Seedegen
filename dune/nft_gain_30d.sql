...
FROM NFT.trades
WHERE (original_currency = 'ETH' OR original_currency = 'WETH') AND block_time >= NOW() - INTERVAL '1 MONTH' AND "nft_contract_address" != '\xb47e3cd837ddf8e4c57f05d70ab865de6e193bbb'
GROUP BY seller), 
...
ELSE ROUND(((sales.sum - buys.sum)/buys.sum), 2) END AS "ROI %(ETH)",
